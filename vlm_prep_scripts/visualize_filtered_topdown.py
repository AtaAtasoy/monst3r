"""
Visualize a normalized filtered scene from a top-down camera.
...
Usage:
    # Joint layout
    python visualize_filtered_topdown.py --data_dir .../normalized --show both
    python visualize_filtered_topdown.py --data_dir .../normalized --show fg --render

    # Independent layout (two flat dirs)
    python visualize_filtered_topdown.py --fg_dir .../normalized_foreground_points --render
    python visualize_filtered_topdown.py --bg_dir .../normalized_background_points --render
    python visualize_filtered_topdown.py --fg_dir .../normalized_foreground_points --bg_dir .../normalized_background_points --show both --render
"""
...
def main(args):
    ...
    if args.render:
        ...
        render_dir = root_dir / f"top-down-renders-{suffix}"
        ...
        for idx, item in enumerate(frame_pcds):
            ...
            if item is None:
                ...
                for m in item:
                    rp.add_mesh(m, rgb=True)
                rendered = render_from_camera(rp, topdown_pose, intr)
            else:
                rp.add_mesh(item, rgb=True)
                rendered = render_from_camera(rp, topdown_pose, intr)
            rp.close()
