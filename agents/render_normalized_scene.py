...
def save_depth_outputs(depth_map: np.ndarray, depth_npy_path: Path, depth_png_path: Path) -> None:
    ...
    if np.any(valid):
        ...
        if depth_max > depth_min:
            depth_vis[valid] = (
                (valid_depth - depth_min) / (depth_max - depth_min) * 255.0
            ).astype(np.uint8)
        else:
            depth_vis[valid] = 255

    cv2.imwrite(str(depth_png_path), depth_vis)
...
def main():
    ...
    depth_vis_dir = output_dir / "depth_viz"
    ...
    for i in range(num_frames):
        ...
        if torch.any(valid):
            camera_space_points = camera.get_world_to_view_transform().transform_points(
                points[None]
            )[0]
            point_depths = camera_space_points[:, 2]
            depth_values = point_depths[visible_idx[valid]].detach().cpu().numpy()
            depth_map[valid.cpu().numpy()] = depth_values.astype(np.float32)

        save_depth_outputs(
            depth_map,
            depth_dir / f"frame_{i:04d}.npy",
            depth_vis_dir / f"{i:04d}.png",
        )

    video_dir = output_dir / "vis-videos"
    capture_video_path = video_dir / "capture-cameras.mp4"
    depth_video_path = video_dir / "depth.mp4"
    create_video_from_pngs(output_dir, capture_video_path, fps=25)
    create_video_from_pngs(depth_vis_dir, depth_video_path, fps=25)

    print(f"\nSaved {num_frames} renders to {output_dir}")
    print(f"Saved depth maps to {depth_dir}")
    print(f"Saved depth visualizations to {depth_vis_dir}")
